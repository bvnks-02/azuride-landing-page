...
type GravityStarsProps = {
  ...
  /** Explicit star color (hex/css). Falls back to the container's computed
   *  `color` when omitted. */
  color?: string;
...
function GravityStarsBackground({
  ...
  // Self-referential animation loop — use a ref to break the circular
  // `useCallback` self-reference (react-hooks/immutability flags the
  // direct self-call). The ref is assigned in an effect below.
